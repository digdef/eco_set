@extends('index')
@section('meta')
    <title>{{ $ceo_text->meta_title ?? 'Комплектующие' }}</title>
    <meta name="description" lang="ru" content="{{ $ceo_text->meta_description ?? 'Комплектующие' }}">

    <meta property="og:title" content="{{ $ceo_text->meta_title ?? 'Комплектующие' }}">
    <meta property="og:type" content="ДСВ – Інновації для Вашого успіху">
    <meta property="og:description" content="{{ $ceo_text->meta_description ?? 'Комплектующие' }}">
        ...
        <div class="catalog-open">
            <div class="wrapper-m wrapper">
                ...
                <form class="filter-sorting filter-sorting-pc">
                    @csrf
                    <p>Сортировать по:</p>
                    <input type="hidden" id="price_filter" name="price_filter" value="{{ $orderBy_name == 'price' && $orderBy_val == 'desc' ? 'asc' : 'desc' }}">
                    <input type="hidden" id="title_filter" name="title_filter" value="{{ $orderBy_name == 'title' && $orderBy_val == 'desc' ? 'asc' : 'desc' }}">
                    <a href="#" class="price_filter {{ $orderBy_name == 'price' && $orderBy_val == 'desc' ? '' : 'arrow-down' }}">Цене <img src="/images/arrow-btm2.svg" alt="arrow-btm2"></a>
                    <a href="#" class="title_filter {{ $orderBy_name == 'title' && $orderBy_val == 'desc' ? '' : 'arrow-down' }}">Названию <img src="/images/arrow-btm2.svg" alt="arrow-btm2"></a>
                </form>
